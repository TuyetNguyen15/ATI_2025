...
def get_notifications():
    ...
    try:
        ...
        # Sắp xếp sau khi lấy dữ liệu (tránh lỗi missing index)
        sorted_docs = sorted(
            docs, 
            key=lambda x: x.to_dict().get("createdAt", datetime.min),
            reverse=True
        )[:50]  # Lấy 50 thông báo mới nhất
...
def breakup():
    ...
    try:
        ...
        notification = {
            ...
            "message": "Mối quan hệ của bạn đã kết thúc",
            "read": False,
            "navigable": False,
            "createdAt": firestore.SERVER_TIMESTAMP,
        }
...
def love_matching_single(match_type):
    try:
        ...
        # --- PROMPT ---
        prompt = f"""
Phân tích loại {match_type}.
Trả về JSON dạng LIST 5 người, không văn bản thừa.
...
"""
        ...
        doc_ref = (
            ...
            .collection("types")       # folder types
            .document(match_type)      # type_name
        )
        ...
        print("💾 FIRESTORE SAVE OK:", match_type)
